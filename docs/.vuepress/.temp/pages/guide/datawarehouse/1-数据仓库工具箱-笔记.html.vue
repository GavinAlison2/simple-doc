<template><div><h1 id="数据仓库工具箱-笔记" tabindex="-1"><a class="header-anchor" href="#数据仓库工具箱-笔记"><span>数据仓库工具箱-笔记</span></a></h1>
<blockquote>
...
<blockquote>
<p>OLTP-保存数据：<br>
面向用户的应用开发，比如网站后台数据库，MySQL等概念<br>
不维护历史数据，仅保留最新的系统状态<br>
强事务性，更快的处理事务，一次处理一个事务</p>
<p>OLAP-分析数据：</p>
<p>面向数据分析的开发，数据分析，大数据、数仓的概念<br>
维护历史数据，数据量大<br>
一次处理多个事务</p>
</blockquote>
<ul>
...
<p>事实表是稀疏的，但是将消耗维度模型的90%甚至更多的空间。</p>
<p>事实表在行数上趋向于边长，在列上趋向于变短</p>
<p>事实表按照事务可划分为分为三类：</p>
<ul>
<li>事务（最常见）</li>
<li>周期性快照</li>
...
<p>维度表由单一主键定义，用于被事实表关联参照。</p>
<p>维度表示查询约束（where语句）、分组（group by）、报表标识的主要来源。属性名一般为名词。可以帮助对系统的理解。尽量使用文本描述而不是代码。</p>
<p>区分数值属性是事实表还是维度表的方法是：</p>
<ul>
<li>如果一个数值属性包含多个值并作为计算参与者的度量——事实</li>
<li>如果仅是对具体值的描述，一个常量、某个约束和行标识的参与者——维度属性</li>
</ul>
