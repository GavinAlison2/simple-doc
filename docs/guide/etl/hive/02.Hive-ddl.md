...
CREATE DATABASE IF NOT EXISTS hive_test
  ...
  WITH DBPROPERTIES ('create'='alison');
```
...
-- load data 
-- 如果在容器内，需将文件传输到容器内
load data local inpath "file:///tmp/emp.txt" into table emp;
...
-- 复制表结构
create table if not EXISTS test.emp_copy_2 like emp;

desc  test. emp_copy_2;
select * from emp_copy_2;

alter table emp_copy_2 rename to emp_copy_new;

show tables;

desc emp_copy_new;
-- 改变表的注释
alter table emp_copy_new set serdeproperties ('serialization.format'='1');
-- 增加列
alter table emp_copy_new add COLUMNs (col1 string comment 'new column', col2 string comment 'new column2');
-- 改变列名 需要兼容 其他类型转string, string不可转其他类型
alter table emp_copy_new CHANGE col1 col11 string comment 'col1 -> col11';
-- 改变列的位置
alter table emp_copy_new change col11 col12 string comment 'col11 -> col12' after col2;


-- 删除表
truncate table emp_copy_new;
drop table emp_copy_new;
